...
class AuthService(
    ...
    fun register(request: RegisterRequest): AuthResponse {
        try {
            println("[DEBUG] Register request: $request")
            // Email required
            if (request.email.isNullOrBlank()) {
                throw IllegalArgumentException("Email address must be provided.")
            }
            ...
            println("[DEBUG] Sending OTP email to ${request.email} with OTP: $otp")
            ...
            println("[DEBUG] User saved: $saved")
            return AuthResponse(
                id = saved.id,
                name = saved.name,
                email = saved.email,
            ...
            println("[ERROR] Registration failed: ${ex.message}")
            ex.printStackTrace()
            throw ex
        }
    }
