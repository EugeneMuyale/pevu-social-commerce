...
class MediaService(
    ...
    fun uploadVideo(file: MultipartFile, uploadSource: String = "product"): MediaUploadResponse {
        ...
        try {
            ...
            val videoType = when {
                uploadSource == "livestream" -> {
                    if (duration < 60 || duration > 180) throw IllegalArgumentException("Live stream must be 60-180 seconds")
            ...
            // If this is a livestream, convert to HLS using ffmpeg
            if (videoType == VideoType.LIVE_STREAM) {
                try {
                    println("[HLS] Starting ffmpeg for $fileName, input: ${tempFile.absolutePath}")
                    val streamKey = fileName.substringBeforeLast('.')
                    val hlsDir = "/opt/data/hls/$streamKey"
                    val hlsPlaylist = "$hlsDir/$streamKey.m3u8"
                    val ffmpegCmd = arrayOf(
                        "ffmpeg", "-y", "-i", tempFile.absolutePath,
                        "-c:v", "libx264", "-c:a", "aac", "-strict", "-2",
                        "-f", "hls", "-hls_time", "4", "-hls_playlist_type", "vod",
                        "-hls_segment_filename", "$hlsDir/${streamKey}_%03d.ts",
                        hlsPlaylist
                    )
                    println("[HLS] ffmpeg command: ${ffmpegCmd.joinToString(" ")}")
                    val hlsDirFile = java.io.File(hlsDir)
                    hlsDirFile.mkdirs()
                    println("[HLS] HLS output directory created: $hlsDir")
                    val process = ProcessBuilder(*ffmpegCmd)
                        .redirectErrorStream(true)
                        .start()
                    val output = process.inputStream.bufferedReader().readText()
                    val exitCode = process.waitFor()
                    println("[HLS] ffmpeg finished for $fileName with exit code $exitCode")
                    if (exitCode != 0) {
                        println("[HLS] ffmpeg failed for $fileName: $output")
                    } else {
                        println("[HLS] ffmpeg succeeded for $fileName, output: $hlsPlaylist")
                    }
                } catch (ex: Exception) {
                    println("[HLS] Exception during ffmpeg HLS conversion: ${ex.message}")
                }
            }
            tempFile.delete()
